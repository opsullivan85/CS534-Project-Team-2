...
def display_parameters(
    batch_size, starting_lr, ema_decay, weight, method, degree, label
):
    """See parameters"""
...
def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    ...
    with tf.variable_scope("discriminator_model"):
        ...
        l_generator, inter_layer_rct = dis(
            z, x_gen, is_training=is_training_pl, reuse=True
        )
    ...
    with tf.name_scope("optimizers"):
        ...
        optimizer_enc = tf.train.AdamOptimizer(
            learning_rate=learning_rate, beta1=0.5, name="enc_optimizer"
        )
    ...
    with tf.variable_scope("encoder_model"):
        z_gen_ema = enc(
            input_pl, is_training=is_training_pl, getter=get_getter(enc_ema), reuse=True
        )
    ...
    with tf.variable_scope("discriminator_model"):
        l_encoder_ema, inter_layer_inp_ema = dis(
            z_gen_ema,
            input_pl,
            is_training=is_training_pl,
        ...
        l_generator_ema, inter_layer_rct_ema = dis(
            z_gen_ema,
            reconstruct_ema,
            is_training=is_training_pl,
            getter=get_getter(dis_ema),
    ...
    sv = tf.train.Supervisor(
        logdir=logdir, save_summaries_secs=None, save_model_secs=120
    )
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            for t in range(nr_batches_train):
                ...
                _, ld, sm = sess.run(
                    [train_dis_op, loss_discriminator, sum_op_dis], feed_dict=feed_dict
                )
                train_loss_dis += ld
        ...
        for t in range(nr_batches_test):
            ...
            feed_dict = {input_pl: testx[ran_from:ran_to], is_training_pl: False}
        ...
        prc_auc = do_prc(
            scores,
            testy,
            file_name=r"bigan/mnist/{}/{}/{}".format(method, weight, label),
            directory=r"results/bigan/mnist/{}/{}/".format(method, weight),
        )
