...
from src.naive_bayes.naive_bayes import run as _, pre_process_data
...
def run_demo(
    ...
    for i in range(num_iterations):
        ...
        if visualize and i >= window_size:
            # get predictions
            X, y = load_most_recent_data(file_name)
            X, y = pre_process_data(X, y)
            pred = np.zeros_like(y) # The GAN always guesses the data is real, it can only distinguish between random noise at this point
